...
public class XMemcachedClientBuilderProxy implements MemcachedClientBuilder {
	...
	public XMemcachedClientBuilderProxy(String addressList) {
		this(AddrUtil.getAddresses(addressList));
	}

	public XMemcachedClientBuilderProxy(List<InetSocketAddress> addressList) {
		if (addressList != null) {
	...
	public XMemcachedClientBuilderProxy(List<InetSocketAddress> addressList, int[] weights) {
		if (addressList != null) {
			for (InetSocketAddress addr : addressList) {
				this.addressMap.put(addr, null);
			}
	...
	public XMemcachedClientBuilderProxy(Map<InetSocketAddress, InetSocketAddress> addressMap) {
		this.addressMap = addressMap;
	}

	public XMemcachedClientBuilderProxy(Map<InetSocketAddress, InetSocketAddress> addressMap, int[] weights) {
		this.addressMap = addressMap;
		this.weights = weights;
	}

	public XMemcachedClientBuilderProxy() {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see net.rubyeye.xmemcached.MemcachedClientBuilder#setSessionLocator(net.rubyeye .xmemcached.MemcachedSessionLocator)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see net.rubyeye.xmemcached.MemcachedClientBuilder#setBufferAllocator(net. rubyeye.xmemcached.buffer.BufferAllocator)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see net.rubyeye.xmemcached.MemcachedClientBuilder#setConfiguration(com.google .code.yanf4j.config.Configuration)
	 */
	...
	public MemcachedClient build() throws IOException {
		XMemcachedClientProxy memcachedClient;
		...
		if (this.commandFactory.getProtocol() == Protocol.Kestrel) {
			if (!(this.sessionLocator instanceof RandomMemcachedSessionLocaltor)) {
			    LogManager.warn("Recommend to use `net.rubyeye.xmemcached.impl.RandomMemcachedSessionLocaltor` as session locator for kestrel protocol.");
			}
		}
		if (this.weights == null) {
			memcachedClient = new XMemcachedClientProxy(this.sessionLocator, this.bufferAllocator, this.configuration, this.socketOptions, this.commandFactory,
					this.transcoder, this.addressMap, this.stateListeners, this.authInfoMap, this.connectionPoolSize, this.connectTimeout, this.name,
					this.failureMode);
			...
			memcachedClient = new XMemcachedClientProxy(this.sessionLocator, this.bufferAllocator, this.configuration, this.socketOptions, this.commandFactory,
					this.transcoder, this.addressMap, this.weights, this.stateListeners, this.authInfoMap, this.connectionPoolSize, this.connectTimeout,
