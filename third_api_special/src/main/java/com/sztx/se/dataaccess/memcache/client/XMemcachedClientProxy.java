...
public class XMemcachedClientProxy implements XMemcachedClientMBean, MemcachedClient {
	...
	public void setTimeoutExceptionThreshold(int timeoutExceptionThreshold) {
		...
		if (timeoutExceptionThreshold < 100) {
			LogManager.warn("Too small timeoutExceptionThreshold value may cause connections disconnect/reconnect frequently.");
		}
	...
	private final <T> GetsResponse<T> gets0(final String key, final byte[] keyBytes, final Transcoder<T> transcoder) throws MemcachedException,
			TimeoutException, InterruptedException {
		GetsResponse<T> result = (GetsResponse<T>) this.fetch0(key, keyBytes, CommandType.GETS_ONE, this.opTimeout, transcoder);
		return result;
	}
	 ...
	 * 
	 * @param server 锟斤拷锟斤拷锟絇
	 * @param port 锟斤拷锟斤拷銊э拷锟斤拷
	 * @throws IOException
	 */
	public XMemcachedClientProxy(final String server, final int port) throws IOException {
		this(server, port, 1);
	}
	 ...
	 * @throws IOException
	 */
	public XMemcachedClientProxy(final String host, final int port, int weight) throws IOException {
		super();
		if (weight <= 0) {
			throw new IllegalArgumentException("weight<=0");
		}
		this.checkServerPort(host, port);
		this.buildConnector(new ArrayMemcachedSessionLocator(), new SimpleBufferAllocator(), XMemcachedClientBuilder.getDefaultConfiguration(),
				XMemcachedClientBuilder.getDefaultSocketOptions(), new TextCommandFactory(), new SerializingTranscoder());
		this.start0();
	...
	public final void addServer(final InetSocketAddress inetSocketAddress, int weight) throws IOException {
	...
	private void connect(final InetSocketAddressWrapper inetSocketAddressWrapper) throws IOException {
		...
		if (this.connectionPoolSize > 1) {
			LogManager.warn("You are using connection pool for xmemcached client,it's not recommended unless you have test it that it can boost performance in your app.");
		}
		for (int i = 0; i < this.connectionPoolSize; i++) {
			...
			try {
				...
				if (!future.get(this.connectTimeout, TimeUnit.MILLISECONDS)) {
					LogManager.error("connect to " + SystemUtils.getRawAddress(inetSocketAddress) + ":" + inetSocketAddress.getPort() + " fail");
				} else {
					connected = true;
				}
				...
				throwable = e;
				LogManager.error("connect to " + SystemUtils.getRawAddress(inetSocketAddress) + ":" + inetSocketAddress.getPort() + " error", e);
			} catch (TimeoutException e) {
				throwable = e;
				LogManager.error("connect to " + SystemUtils.getRawAddress(inetSocketAddress) + ":" + inetSocketAddress.getPort() + " timeout", e);
			} catch (Exception e) {
				throwable = e;
				LogManager.error("connect to " + SystemUtils.getRawAddress(inetSocketAddress) + ":" + inetSocketAddress.getPort() + " error", e);
			}
			...
			if (!connected) {
				...
				LogManager.error("Connect to " + SystemUtils.getRawAddress(inetSocketAddress) + ":" + inetSocketAddress.getPort() + " fail", throwable);
	...
	@SuppressWarnings("unchecked")
	private final <T> Object fetch0(final String key, final byte[] keyBytes, final CommandType cmdType, final long timeout, Transcoder<T> transcoder)
			throws InterruptedException, TimeoutException, MemcachedException, MemcachedException {
		final Command command = this.commandFactory.createGetCommand(key, keyBytes, cmdType, this.transcoder);
	...
	private final void startConnector() throws IOException {
		if (this.shutdown) {
			...
			this.shutdownHookThread = new Thread() {
				...
				public void run() {
					try {
						XMemcachedClientProxy.this.isHutdownHookCalled = true;
						XMemcachedClientProxy.this.shutdown();
					} catch (IOException e) {
						LogManager.error("Shutdown XMemcachedClient error", e);
					}
				}
			};
	...
	@SuppressWarnings({ "rawtypes" })
	private void buildConnector(MemcachedSessionLocator locator, BufferAllocator bufferAllocator, Configuration configuration,
		...
		LogManager.warn("XMemcachedClient is using " + this.commandFactory.getProtocol().name() + " protocol");
		this.commandFactory.setBufferAllocator(bufferAllocator);
		this.shutdown = true;
		this.transcoder = transcoder;
		this.sessionLocator = locator;
		this.connector = this.newConnector(bufferAllocator, configuration, this.sessionLocator, this.commandFactory, this.connectionPoolSize,
		...
		if (this.isFailureMode()) {
			LogManager.warn("XMemcachedClient in failure mode.");
		}
		this.connector.setFailureMode(this.failureMode);
		this.sessionLocator.setFailureMode(this.failureMode);
	}

	protected MemcachedConnector newConnector(BufferAllocator bufferAllocator, Configuration configuration, MemcachedSessionLocator memcachedSessionLocator,
	...
	private final void registerMBean() {
		if (this.shutdown) {
			XMemcachedMbeanServer.getInstance().registMBean(this,
					this.getClass().getPackage().getName() + ":type=" + this.getClass().getSimpleName() + "-" + MemcachedClientNameHolder.getName());
		}
	}
	 ...
	 * @throws IOException
	 */
	public XMemcachedClientProxy(final InetSocketAddress inetSocketAddress, int weight) throws IOException {
		super();
		if (inetSocketAddress == null) {
			throw new IllegalArgumentException("Null InetSocketAddress");
	...
	public XMemcachedClientProxy(final InetSocketAddress inetSocketAddress) throws IOException {
		this(inetSocketAddress, 1);
	}

	public XMemcachedClientProxy() throws IOException {
		super();
		this.buildConnector(new ArrayMemcachedSessionLocator(), new SimpleBufferAllocator(), XMemcachedClientBuilder.getDefaultConfiguration(),
				XMemcachedClientBuilder.getDefaultSocketOptions(), new TextCommandFactory(), new SerializingTranscoder());
		this.start0();
	 ...
	 */
	@SuppressWarnings({ "rawtypes" })
	XMemcachedClientProxy(MemcachedSessionLocator locator, BufferAllocator allocator, Configuration conf, Map<SocketOption, Object> socketOptions,
			CommandFactory commandFactory, Transcoder transcoder, Map<InetSocketAddress, InetSocketAddress> addressMap,
			List<MemcachedClientStateListener> stateListeners, Map<InetSocketAddress, AuthInfo> map, int poolSize, long connectTimeout, String name,
	...
	@SuppressWarnings({ "rawtypes" })
	XMemcachedClientProxy(MemcachedSessionLocator locator, BufferAllocator allocator, Configuration conf, Map<SocketOption, Object> socketOptions,
			CommandFactory commandFactory, Transcoder transcoder, Map<InetSocketAddress, InetSocketAddress> addressMap, int[] weights,
			List<MemcachedClientStateListener> stateListeners, Map<InetSocketAddress, AuthInfo> infoMap, int poolSize, long connectTimeout, final String name,
			boolean failureMode) throws IOException {
		super();
		this.setConnectTimeout(connectTimeout);
		this.setFailureMode(failureMode);
	...
	private final void optimiezeSetReadThreadCount(Configuration conf, int addressCount) {
		if (conf != null && addressCount > 1) {
			if (!this.isWindowsPlatform() && conf.getReadThreadCount() == DEFAULT_READ_THREAD_COUNT) {
	 ...
	 * @throws IOException
	 */
	public XMemcachedClientProxy(List<InetSocketAddress> addressList) throws IOException {
		super();
		...
		this.buildConnector(new ArrayMemcachedSessionLocator(), simpleBufferAllocator, XMemcachedClientBuilder.getDefaultConfiguration(),
				XMemcachedClientBuilder.getDefaultSocketOptions(), new TextCommandFactory(), new SerializingTranscoder());
		this.start0();
	 ...
	 */
	@SuppressWarnings("unchecked")
	public final <T> T get(final String key, final long timeout, final Transcoder<T> transcoder) throws TimeoutException, InterruptedException,
			MemcachedException {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public final <T> T get(final String key, final long timeout) throws TimeoutException, InterruptedException, MemcachedException {
	 ...
	 */
	public final <T> T get(final String key, final Transcoder<T> transcoder) throws TimeoutException, InterruptedException, MemcachedException {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public final <T> T get(final String key) throws TimeoutException, InterruptedException, MemcachedException {
		return (T) this.get(key, this.opTimeout);
	}

	private <T> Object get0(String key, final long timeout, final CommandType cmdType, final Transcoder<T> transcoder) throws TimeoutException,
	 ...
	 */
	@SuppressWarnings("unchecked")
	public final <T> GetsResponse<T> gets(final String key, final long timeout, final Transcoder<T> transcoder) throws TimeoutException, InterruptedException,
			MemcachedException {
	 ...
	 */
	public final <T> GetsResponse<T> gets(final String key) throws TimeoutException, InterruptedException, MemcachedException {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public final <T> GetsResponse<T> gets(final String key, final long timeout) throws TimeoutException, InterruptedException, MemcachedException {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public final <T> GetsResponse<T> gets(final String key, @SuppressWarnings("rawtypes") final Transcoder transcoder) throws TimeoutException,
			InterruptedException, MemcachedException {
	 ...
	 */
	public final <T> Map<String, T> get(final Collection<String> keyCollections, final long timeout, final Transcoder<T> transcoder) throws TimeoutException,
			InterruptedException, MemcachedException {
	 ...
	 */
	public final <T> Map<String, T> get(final Collection<String> keyCollections, final Transcoder<T> transcoder) throws TimeoutException, InterruptedException,
			MemcachedException {
	 ...
	 */
	public final <T> Map<String, T> get(final Collection<String> keyCollections) throws TimeoutException, InterruptedException, MemcachedException {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public final <T> Map<String, T> get(final Collection<String> keyCollections, final long timeout) throws TimeoutException, InterruptedException,
			MemcachedException {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public final <T> Map<String, GetsResponse<T>> gets(final Collection<String> keyCollections, final long timeout, final Transcoder<T> transcoder)
			throws TimeoutException, InterruptedException, MemcachedException {
		return (Map<String, GetsResponse<T>>) this.getMulti0(keyCollections, timeout, CommandType.GETS_MANY, transcoder);
	}
	 ...
	 */
	public final <T> Map<String, GetsResponse<T>> gets(final Collection<String> keyCollections) throws TimeoutException, InterruptedException,
			MemcachedException {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public final <T> Map<String, GetsResponse<T>> gets(final Collection<String> keyCollections, final long timeout) throws TimeoutException,
			InterruptedException, MemcachedException {
	 ...
	 */
	public final <T> Map<String, GetsResponse<T>> gets(final Collection<String> keyCollections, final Transcoder<T> transcoder) throws TimeoutException,
			InterruptedException, MemcachedException {
		return this.gets(keyCollections, this.opTimeout, transcoder);
	}

	private final <T> Map<String, T> getMulti0(final Collection<String> keys, final long timeout, final CommandType cmdType, final Transcoder<T> transcoder)
			throws TimeoutException, InterruptedException, MemcachedException {
	...
	@SuppressWarnings("unchecked")
	private <T> Map<String, T> reduceResult(final CommandType cmdType, final Transcoder<T> transcoder, final List<Command> commands) throws MemcachedException,
		...
		for (Command getCmd : commands) {
			...
			if (cmdType == CommandType.GET_MANY) {
				...
				while (it.hasNext()) {
					Map.Entry<String, CachedData> entry = it.next();
					@SuppressWarnings("rawtypes")
					GetsResponse getsResponse = new GetsResponse(entry.getValue().getCas(), transcoder.decode(entry.getValue()));
					String decodeKey = this.decodeKey(entry.getKey());
					if (decodeKey != null) {
	...
	private final <T> Command sendGetMultiCommand(final Collection<String> keys, final CountDownLatch latch, final CommandType cmdType,
			final Transcoder<T> transcoder) throws InterruptedException, TimeoutException, MemcachedException {
		final Command command = this.commandFactory.createGetMultiCommand(keys, latch, cmdType, transcoder);
	 ...
	 */
	public final <T> boolean set(String key, final int exp, final T value, final Transcoder<T> transcoder, final long timeout) throws TimeoutException,
			InterruptedException, MemcachedException {
		key = this.preProcessKey(key);
	 ...
	 * @see net.rubyeye.xmemcached.MemcachedClient#set(java.lang.String, int, java.lang.Object)
	 */
	public final boolean setBroadcast(String key, final int exp, final Object value) throws TimeoutException, InterruptedException, MemcachedException {
		...
		for (Session session : sessions) {
			if (session != null && !session.isClosed()) {
				Command command = this.commandFactory.createSetCommand(key, keyBytes, exp, value, false, transcoder);
				flag = this.sendStoreCommand(session, command, this.opTimeout);
			} else {
				latch.countDown();
			}
		}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public final boolean set(final String key, final int exp, final Object value, final long timeout) throws TimeoutException, InterruptedException,
			MemcachedException {
	 ...
	 */
	public final <T> boolean set(final String key, final int exp, final T value, final Transcoder<T> transcoder) throws TimeoutException, InterruptedException,
			MemcachedException {
	 ...
	 */
	public final <T> boolean add(String key, final int exp, final T value, final Transcoder<T> transcoder, final long timeout) throws TimeoutException,
			InterruptedException, MemcachedException {
		key = this.preProcessKey(key);
		return this.add0(key, exp, value, transcoder, timeout);
	}

	private <T> boolean add0(String key, int exp, T value, Transcoder<T> transcoder, long timeout) throws InterruptedException, TimeoutException,
			MemcachedException {
		byte[] keyBytes = this.checkStoreArguments(key, exp, value);
	 ...
	 */
	public final boolean add(final String key, final int exp, final Object value) throws TimeoutException, InterruptedException, MemcachedException {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public final boolean add(final String key, final int exp, final Object value, final long timeout) throws TimeoutException, InterruptedException,
			MemcachedException {
	 ...
	 */
	public final <T> boolean add(final String key, final int exp, final T value, final Transcoder<T> transcoder) throws TimeoutException, InterruptedException,
			MemcachedException {
	 ...
	 */
	public final <T> boolean replace(String key, final int exp, final T value, final Transcoder<T> transcoder, final long timeout) throws TimeoutException,
			InterruptedException, MemcachedException {
		key = this.preProcessKey(key);
		byte[] keyBytes = this.checkStoreArguments(key, exp, value);
	 ...
	 */
	public final boolean replace(final String key, final int exp, final Object value) throws TimeoutException, InterruptedException, MemcachedException {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public final boolean replace(final String key, final int exp, final Object value, final long timeout) throws TimeoutException, InterruptedException,
			MemcachedException {
	 ...
	 */
	public final <T> boolean replace(final String key, final int exp, final T value, final Transcoder<T> transcoder) throws TimeoutException,
			InterruptedException, MemcachedException {
	 ...
	 */
	public final boolean append(final String key, final Object value) throws TimeoutException, InterruptedException, MemcachedException {
	 ...
	 */
	public final boolean append(String key, final Object value, final long timeout) throws TimeoutException, InterruptedException, MemcachedException {
		key = this.preProcessKey(key);
	 ...
	 */
	public final boolean prepend(final String key, final Object value) throws TimeoutException, InterruptedException, MemcachedException {
	 ...
	 */
	public final boolean prepend(String key, final Object value, final long timeout) throws TimeoutException, InterruptedException, MemcachedException {
		key = this.preProcessKey(key);
		byte[] keyBytes = this.checkStoreArguments(key, 0, value);
	 ...
	 */
	public final boolean cas(final String key, final int exp, final Object value, final long cas) throws TimeoutException, InterruptedException,
			MemcachedException {
	 ...
	 */
	public final <T> boolean cas(String key, final int exp, final T value, final Transcoder<T> transcoder, final long timeout, final long cas)
			throws TimeoutException, InterruptedException, MemcachedException {
		key = this.preProcessKey(key);
		byte[] keyBytes = this.checkStoreArguments(key, 0, value);
	 ...
	 */
	@SuppressWarnings("unchecked")
	public final boolean cas(final String key, final int exp, final Object value, final long timeout, final long cas) throws TimeoutException,
			InterruptedException, MemcachedException {
	 ...
	 */
	public final <T> boolean cas(final String key, final int exp, final T value, final Transcoder<T> transcoder, final long cas) throws TimeoutException,
			InterruptedException, MemcachedException {
		return this.cas(key, exp, value, transcoder, this.opTimeout, cas);
	}

	private final <T> boolean cas0(final String key, final int exp, GetsResponse<T> getsResponse, final CASOperation<T> operation,
			final Transcoder<T> transcoder, byte[] keyBytes, boolean noreply) throws TimeoutException, InterruptedException, MemcachedException {
		if (operation == null) {
		...
		while (tryCount <= operation.getMaxTries()
				&& result != null
				&& !this.sendStoreCommand(this.commandFactory.createCASCommand(key, keyBytes, exp, operation.getNewValue(result.getCas(), result.getValue()),
						result.getCas(), noreply, transcoder), this.opTimeout) && !noreply) {
			tryCount++;
			result = this.gets0(key, keyBytes, transcoder);
			if (result == null) {
				throw new MemcachedException("could not gets the value for Key=" + key + " for cas");
			}
	 ...
	 */
	public final <T> boolean cas(String key, final int exp, final CASOperation<T> operation, final Transcoder<T> transcoder) throws TimeoutException,
			InterruptedException, MemcachedException {
		key = this.preProcessKey(key);
		byte[] keyBytes = ByteUtils.getBytes(key);
		ByteUtils.checkKey(keyBytes);
	 ...
	 */
	public final <T> boolean cas(String key, final int exp, GetsResponse<T> getsReponse, final CASOperation<T> operation, final Transcoder<T> transcoder)
			throws TimeoutException, InterruptedException, MemcachedException {
		key = this.preProcessKey(key);
		byte[] keyBytes = ByteUtils.getBytes(key);
		ByteUtils.checkKey(keyBytes);
	 ...
	 */
	@SuppressWarnings("unchecked")
	public final <T> boolean cas(final String key, final int exp, GetsResponse<T> getsReponse, final CASOperation<T> operation) throws TimeoutException,
			InterruptedException, MemcachedException {
	...
	public <T> void casWithNoReply(String key, GetsResponse<T> getsResponse, CASOperation<T> operation) throws TimeoutException, InterruptedException,
			MemcachedException {
	...
	@SuppressWarnings("unchecked")
	public <T> void casWithNoReply(String key, int exp, GetsResponse<T> getsReponse, CASOperation<T> operation) throws TimeoutException, InterruptedException,
			MemcachedException {
		key = this.preProcessKey(key);
		byte[] keyBytes = ByteUtils.getBytes(key);
		ByteUtils.checkKey(keyBytes);
	 ...
	 */
	public final <T> boolean cas(final String key, GetsResponse<T> getsReponse, final CASOperation<T> operation) throws TimeoutException, InterruptedException,
			MemcachedException {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public final <T> boolean cas(final String key, final int exp, final CASOperation<T> operation) throws TimeoutException, InterruptedException,
			MemcachedException {
	 ...
	 */
	public final <T> boolean cas(final String key, final CASOperation<T> operation) throws TimeoutException, InterruptedException, MemcachedException {
	 ...
	 */
	public final boolean delete(final String key, final int time) throws TimeoutException, InterruptedException, MemcachedException {
	 ...
	 */
	public final void deleteWithNoReply(final String key, final int time) throws InterruptedException, MemcachedException {
		try {
	...
	public final void deleteWithNoReply(final String key) throws InterruptedException, MemcachedException {
		this.deleteWithNoReply(key, 0);
	}

	private boolean delete0(String key, final int time, long cas, boolean noreply, long opTimeout) throws MemcachedException, InterruptedException,
			TimeoutException {
		key = this.preProcessKey(key);
		final byte[] keyBytes = ByteUtils.getBytes(key);
		ByteUtils.checkKey(keyBytes);
		final Command command = this.commandFactory.createDeleteCommand(key, keyBytes, time, cas, noreply);
	...
	public boolean touch(String key, int exp, long opTimeout) throws TimeoutException, InterruptedException, MemcachedException {
		...
		final Command command = this.commandFactory.createTouchCommand(key, keyBytes, latch, exp, false);
	...
	public <T> T getAndTouch(String key, int newExp, long opTimeout) throws TimeoutException, InterruptedException, MemcachedException {
		...
		ByteUtils.checkKey(keyBytes);
		CountDownLatch latch = new CountDownLatch(1);
		final Command command = this.commandFactory.createGetAndTouchCommand(key, keyBytes, latch, newExp, false);
		this.latchWait(command, opTimeout, this.sendCommand(command));
		command.getIoBuffer().free();
		this.checkException(command);
		CachedData data = (CachedData) command.getResult();
	 ...
	 */
	public final long incr(String key, final long delta) throws TimeoutException, InterruptedException, MemcachedException {
		key = this.preProcessKey(key);
		return this.sendIncrOrDecrCommand(key, delta, 0, CommandType.INCR, false, this.opTimeout, 0);
	}
	...
	public final void incrWithNoReply(String key, long delta) throws InterruptedException, MemcachedException {
		key = this.preProcessKey(key);
		try {
			this.sendIncrOrDecrCommand(key, delta, 0, CommandType.INCR, true, this.opTimeout, 0);
		} catch (TimeoutException e) {
			throw new MemcachedException(e);
		}
	}

	public final void decrWithNoReply(String key, final long delta) throws InterruptedException, MemcachedException {
		key = this.preProcessKey(key);
		try {
			this.sendIncrOrDecrCommand(key, delta, 0, CommandType.DECR, true, this.opTimeout, 0);
		} catch (TimeoutException e) {
			throw new MemcachedException(e);
		}
	}
	...
	public final long decr(String key, final long delta) throws TimeoutException, InterruptedException, MemcachedException {
		key = this.preProcessKey(key);
		return this.sendIncrOrDecrCommand(key, delta, 0, CommandType.DECR, false, this.opTimeout, 0);
	}
	...
	private void setMemcachedLoggingLevel(InetSocketAddress address, int level, boolean noreply) throws MemcachedException, InterruptedException,
			TimeoutException {
		if (address == null) {
		...
		if (sessionQueue == null || sessionQueue.peek() == null) {
			throw new MemcachedException("could not find session for " + SystemUtils.getRawAddress(address) + ":" + address.getPort()
	...
	private void flushSpecialMemcachedServer(InetSocketAddress address, long timeout, boolean noreply, int exptime) throws MemcachedException,
			InterruptedException, TimeoutException {
		if (address == null) {
			throw new IllegalArgumentException("Null adderss");
		}
	...
	@SuppressWarnings("unchecked")
	public final Map<InetSocketAddress, Map<String, String>> getStatsByItem(String itemName, long timeout) throws MemcachedException, InterruptedException,
			TimeoutException {
		...
		for (Command command : commands) {
			this.checkException(command);
			collectResult.put(((ServerAddressAware) command).getServer(), (Map<String, String>) command.getResult());
		}
		return collectResult;
	}
	...
	@SuppressWarnings("unchecked")
	private long sendIncrOrDecrCommand(final String key, final long delta, long initValue, final CommandType cmdType, boolean noreply, long operationTimeout,
			int exp) throws InterruptedException, TimeoutException, MemcachedException {
		final byte[] keyBytes = ByteUtils.getBytes(key);
		ByteUtils.checkKey(keyBytes);
		final Command command = this.commandFactory.createIncrDecrCommand(key, keyBytes, delta, initValue, exp, cmdType, noreply);
		final Session session = this.sendCommand(command);
		if (!command.isNoreply()) {
	 ...
	 */
	public final boolean delete(final String key) throws TimeoutException, InterruptedException, MemcachedException {
		return this.delete(key, 0);
	}

	public final boolean deleteBroadcast(String key) throws TimeoutException, InterruptedException, MemcachedException {
		final Collection<Session> sessions = this.connector.getSessionSet();
		CountDownLatch latch = new CountDownLatch(sessions.size());
		key = this.preProcessKey(key);
		final byte[] keyBytes = ByteUtils.getBytes(key);
		...
		for (Session session : sessions) {
			if (session != null && !session.isClosed()) {
				final Command command = this.commandFactory.createDeleteCommand(key, keyBytes, 0, 0, false);
				flag = this.sendDeleteCommand(session, command, this.opTimeout);
			} else {
				latch.countDown();
			}
		}
	 ...
	 */
	public final void setTranscoder(@SuppressWarnings("rawtypes") final Transcoder transcoder) {
		this.transcoder = transcoder;
	}

	private final <T> boolean sendStoreCommand(final Session session, Command command, long timeout) throws InterruptedException, TimeoutException,
			MemcachedException {
		session.write(command);
	...
	private final <T> boolean sendDeleteCommand(final Session session, Command command, long timeout) throws InterruptedException, TimeoutException,
			MemcachedException {
		session.write(command);
	...
	private void latchWait(final Command cmd, final long timeout, final Session session) throws InterruptedException, TimeoutException {
		if (cmd.getLatch().await(timeout, TimeUnit.MILLISECONDS)) {
			...
			if (counter.incrementAndGet() > this.timeoutExceptionThreshold) {
				LogManager.warn(session + " exceeded continuous timeout threshold,we will close it.");
				try {
					// reset counter.
					counter.set(0);
					session.close();
	...
	private AtomicInteger getContinuousTimeoutCounter(final Session session) {
		...
		if (counter == null) {
			...
			AtomicInteger oldCounter = (AtomicInteger) session.setAttributeIfAbsent(CONTINUOUS_TIMEOUT_COUNTER, counter);
			if (oldCounter != null) {
				counter = oldCounter;
			}
		}
	...
	public Collection<MemcachedClientStateListener> getStateListeners() {
		final List<MemcachedClientStateListener> result = new ArrayList<MemcachedClientStateListener>(this.stateListenerAdapters.size());
