...
import com.tuhanbao.web.filter.Selector;
...
import com.tuhanbao.base.dataservice.filter.ISqlItem;
...
public class ServiceImpl<E extends ServiceBean> implements IService<E> {
    ...
    public int delete(Filter filter) {
        ...
        return mapper.deleteByFilter(table, filter);
    }
    ...
    @SuppressWarnings("unchecked")
    protected List<E> select(Selector selector, Filter filter) {
    ...
    private Table getTable(Filter filter) {
        Table table = filter != null ? filter.getTable() : null;
        ...
        List<ISqlItem> items = filter.getItems();
        if (items != null) {
