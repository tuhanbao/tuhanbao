...
public class SqlGeneratorFilter extends ExcelAGCFilter {
        ...
        try {
            ...
            if (ProjectConfig.NEED_SYNC_DB) {
                ...
                for (ImportTable table : getTables(context)) {
                    ...
                    DBSrc dbSrc = ModuleManager.getDBSrc(module);
                ...
                for (Entry<DBSrc, List<ImportTable>> entry : newTables.entrySet()) {
                    ...
                    A : for (ImportTable oldTable : oldTables) {
                        ...
                        moduleSb.get(ModuleManager.getModule(entry.getKey())).append(DBUtil.getDropTableSql(oldTable, entry.getKey().getDBType()));
                    }
                }
                
                SpringMvcProjectInfo project = this.getProject(context);
                for (Entry<String, StringBuilder> entry : moduleSb.entrySet()) {
                    context.addEvent(new CreateFileEvent(getSqlName(project, entry.getKey()), entry.getValue().toString()));
                }
            }
        }
        catch (Exception e) {
    ...
    private String getSqlName(SpringMvcProjectInfo project, String module) {
