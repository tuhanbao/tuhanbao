 ...
 * 查询条件构造器，构建形式如下 .
 * 
 * List<FilterRule> rules = FilterRuleBuilder.newBuilder() .key("name").eq().value("324") .and() .key("password").like().value("fsd") .and() .key("add").in().value(new String[]{"111","222"}) .and()
 * .key("address").ge().value("234") .build();
 * 
 * @author: zhihongp
 * @date: 2014-03-05 15:42
 */
public class FilterRuleBuilder {
	...
	public static void main(String[] args) {
		List<FilterRule> rules = FilterRuleBuilder.newBuilder().key("name").eq().value("324").and().key("password").like().value("fsd").and().key("address").ge().value("234").build();
