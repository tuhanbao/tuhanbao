...
import com.tuhanbao.base.util.db.table.ColumnManager;
...
public class BeanWrapperImpl extends AbstractPropertyAccessor implements BeanWrapper {
	...
	private Map<String, Column> propertyColumns = null;
	
	private static final Method SET_VALUE;

	static {
		try {
			SET_VALUE = ServiceBean.class.getMethod("setValue", Column.class, DataValue.class);
		}
		catch (Exception e) {
	...
	public void setWrappedInstance(Object object, String nestedPath, Object rootObject) {
		...
		this.isMO = this.object instanceof ServiceBean;
		if (this.isMO) {
			Table table = ((ServiceBean)object).getTable();
			propertyColumns = ColumnManager.getTableColumns(table);
		}
	}
	...
	private void setPropertyValue(PropertyTokenHolder tokens, PropertyValue pv) throws BeansException {
		...
		if (tokens.keys != null) {
			...
			if (pd == null || !pd.getWriteMethod().getDeclaringClass().isInstance(this.object)) {
				...
				if (isMO && handleMOProperty(pv, propertyName, pd)) return;
	...
	private boolean handleMOProperty(PropertyValue pv, String propertyName, PropertyDescriptor pd) {
		// 由于jdk的内省找不到mo的setmethod，只能自己覆盖了，
		...
		if (pd == null || pd.getWriteMethod() == null) {
			...
			needHandle = types.length == 1 && types[0] != String.class;
		}
			...
			if (column != null) {
				...
				try {
					if (dataType == DataType.DATE) {
						SET_VALUE.invoke(object, column, DataValueFactory.toDataValue(dataType, Long.valueOf(pv.getValue().toString())));
					}
					else {
						SET_VALUE.invoke(object, column, DataValueFactory.toDataValue(dataType, pv.getValue()));
					}
					return true;
