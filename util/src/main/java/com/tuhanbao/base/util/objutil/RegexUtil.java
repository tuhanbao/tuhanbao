...
public class RegexUtil {
    ...
    /** 12位以内正整数 */
    private static final String V_Z_12Int = "\\d{1,12}$";
    ...
    /** 手机 */
    private static final String V_MOBILE = "^(13|15|18|14)[0-9]{9}$";
    ...
    /** 4位小数验证 \d{1,12}(\.\d{0,4})? */
    private static final String V_4_FLOAT = "^[1-9]+(\\.\\d{0,4})?$|^0(\\.\\d{0,4})?$|^[1-9]+[0-9]*(\\.\\d{0,4})?$";

    /** 验证只有英文或者数字、下划线的字符串 */
    private static final String V_NUMBER_ENG = "^[A-Za-z0-9_-]+$";

    /** 验证只有英文或者数字的字符串 */
    private static final String ONLY_NUMBER_ENG = "^[A-Za-z0-9]+$";

    /** 验证格林尼治时间 */
    private static final String GMT_DATE = "(Mon|Tue|Wed|Thu|Fri|Sat|Sun)\\s(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\\s(\\d{2})\\s([01]\\d|2[0-3]):([0-5]\\d):([0-5]\\d)\\sCST\\s(\\d{4})";
     ...
     */
    public static boolean onlyNumberOrEng(String value) {
        return match(ONLY_NUMBER_ENG, value);
    }

    public static boolean numberOrEng(String value) {
        return match(V_NUMBER_ENG, value);
    }

    /** 验证全角、半角字符,含有全角字符返回false */
     ...
     * @return 如果是符合格式的字符串,返回true ,否则为false
     */
    public static boolean Z_12Int(String value) {
        return match(V_Z_12Int, value);
    }
     ...
     * @return 如果是符合格式的字符串,返回true ,否则为false
     */
    public static boolean fourFloat(String value) {
        return match(V_4_FLOAT, value);
    }

    /** 验证格林尼治时间 */
    public static boolean gmtDate(String value) {
        return match(GMT_DATE, value);
    }
}
