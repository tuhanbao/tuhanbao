...
public class ServiceImpl<E extends ServiceBean> implements IService<E> {
    @Autowired
    protected CrmMapper<ServiceBean> mapper;
    ...
    public int update(E record, Filter filter) {
        ...
        int result = mapper.updateByFilter(record, MyBatisFilter.getInstance(filter));
        return result;
    }
    ...
    public int update(E record) {
        Table table = record.getTable();
        int result = mapper.updateByFilter(record, MyBatisFilter.getInstance(new Filter().andFilter(record.getTable().getPK(), record.getKeyValue())));
        if (table.getCacheType() == CacheType.CACHE_ALL) {
            CacheManager.save(record);
        }
        return result;
    ...
    public int updateSelective(E record, Filter filter) {
        int result = mapper.updateSelective(record, MyBatisFilter.getInstance(filter));
    ...
    public int updateSelective(E record) {
        ...
        int result = mapper.updateSelective(record, MyBatisFilter.getInstance(new Filter().andFilter(record.getTable().getPK(), keyValue)));
        if (record.getTable().getCacheType() == CacheType.CACHE_ALL) {
    ...
    protected Table getTable(Filter filter) {
